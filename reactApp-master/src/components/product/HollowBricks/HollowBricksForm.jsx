...
import {TextField, Button, Grid, Autocomplete, MenuItem, FormControl, InputLabel, Select} from "@mui/material";
...
const HollowBricksForm = ({ companyId, initialHollowBrick, onSuccess, notification }) => {
    const [granite, setMarble] = useState(initialHollowBrick || {});
    const [plants, setPlants] = useState([]);
    const [isLoadingPlants, setIsLoadingPlants] = useState(false);
    const [searchQuery, setSearchQuery] = useState(null);
    const [selectedPlant, setSelectedPlant] = useState(initialHollowBrick?.plant || null);
    ...
    const handleSubmit = async e => {
        ...
        try {
            if (initialHollowBrick) {
                await allProductApi.update("hollowBricks",initialHollowBrick.id, granite);
                notification({ message: 'Marble updated successfully!', severity: 'success' });

            } else {
                await allProductApi.create("hollowBricks",granite);
                ...
                <Grid item xs={12}>
                    <FormControl variant="outlined" fullWidth>
                        ...
                        <Select
                            ...
                            onChange={handleInputChange}
                        >
                            <MenuItem value={'Concrete Hollow Blocks'}>Concrete Hollow Blocks</MenuItem>
                            <MenuItem value={'AAC'}>(AAC) Aerated Autoclaved Concrete Blocks </MenuItem>
                            <MenuItem value={'Clay Hollow Blocks'}>Clay Hollow Blocks</MenuItem>
                            <MenuItem value={'Hollow Sand-lime Bricks'}>Hollow Sand-lime Bricks</MenuItem>
                            <MenuItem value={'Hollow Ceramic Blocks'}>Hollow Ceramic Blocks</MenuItem>
                            <MenuItem value={'Cellular Lightweight Concrete Blocks'}>Cellular Lightweight Concrete Blocks</MenuItem>
                        </Select>
                    </FormControl>
                </Grid>
                <Grid item xs={12}>
...
export default HollowBricksForm;
