...
const MarbleForm = ({ companyId, initialMarble, onSuccess, notification }) => {
    const [marble, setMarble] = useState(initialMarble || {});
    const [plants, setPlants] = useState([]);
    const [isLoadingPlants, setIsLoadingPlants] = useState(false);
    const [searchQuery, setSearchQuery] = useState(null);
    const [selectedPlant, setSelectedPlant] = useState(initialMarble?.plant || null);
    ...
    const handleSubmit = async e => {
        ...
        try {
            if (initialMarble) {
                await allProductApi.update("marbles",initialMarble.id, marble);
                notification({ message: 'Marble updated successfully!', severity: 'success' });

            } else {
                await allProductApi.create("marbles",marble);
                notification({ message: 'Marble added successfully!', severity: 'success' });
            }
            if (onSuccess) onSuccess();
        } catch (error) {
            notification({ message: `Error processing marble: ${error.message}`, severity: 'error' });
        }finally {
