...
import { Button, List, ListItem, ListItemText, IconButton, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
import { Select, MenuItem, FormControl, InputLabel,useTheme ,useMediaQuery,Grid  } from '@mui/material';

const VisuallyHiddenInput = styled('input')({
    display: 'none',
});

function ProductImagesCrud({ product ,notification }) {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
    const [images, setImages] = useState(product ? product.imageUrls || [] : []);
    const [open, setOpen] = useState(false);
    const [selectedFile, setSelectedFile] = useState(null);
    const [previewUrl, setPreviewUrl] = useState();
    const [uploadProgress, setUploadProgress] = useState(0);
    const [description, setDescription] = useState("Product-Image");
    ...
    const handleUploadImage = async () => {
        ...
        try {
            ...
            await productApi.addImage(product.company.id, product.id, uploadedImage.id);
            // console.log("Product updated, updating local state..."+product.company.id + product.id + uploadedImage.id );
    ...
    const handleDeleteImage = async (image) => {
        try {
            await productApi.removeImage(product.company.id, product.id, image);
                    ...
                    <Grid container spacing={2} direction="column">
                        ...
                        <Grid item xs={12}>
                            <Button component="label" variant="contained" startIcon={<CloudUploadIcon />}>
                                Upload File
                                <VisuallyHiddenInput type="file" onChange={handleImageSelection} />
                            </Button>
                        </Grid>
                        <Grid item xs={12}>
